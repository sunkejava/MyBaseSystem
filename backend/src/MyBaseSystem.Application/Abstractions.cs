...
public interface IUserApplicationService { Task<PagedResult<UserListItem>> ListAsync(string? keyword, int page, int pageSize, CancellationToken ct); Task<Guid> CreateAsync(SaveUserRequest request, CancellationToken ct); Task UpdateAsync(Guid id, SaveUserRequest request, CancellationToken ct); Task DeleteAsync(Guid id, CancellationToken ct); Task ChangePasswordAsync(Guid id,ChangePasswordRequest request,CancellationToken ct); }
...
{
    Task<DashboardSummary> SummaryAsync(Guid userId,CancellationToken ct);
    Task<IReadOnlyList<Notification>> NotificationsAsync(Guid userId,CancellationToken ct);
    Task<Notification?> NotificationAsync(Guid id,Guid userId,CancellationToken ct);
    Task<IReadOnlyList<SystemSetting>> SettingsAsync(CancellationToken ct);
    Task<SystemSetting?> SettingAsync(string key,CancellationToken ct);
    void Add(Notification item); void Add(SystemSetting item); Task SaveChangesAsync(CancellationToken ct);
}
public interface IOperationsApplicationService
{
    Task<DashboardSummary> SummaryAsync(Guid userId,CancellationToken ct); Task<IReadOnlyList<NotificationDto>> NotificationsAsync(Guid userId,CancellationToken ct);
    Task<Guid> CreateNotificationAsync(CreateNotificationRequest request,CancellationToken ct); Task MarkReadAsync(Guid id,Guid userId,CancellationToken ct); Task MarkAllReadAsync(Guid userId,CancellationToken ct); Task DeleteNotificationAsync(Guid id,Guid userId,CancellationToken ct);
    Task<IReadOnlyList<SettingDto>> SettingsAsync(CancellationToken ct); Task SaveSettingAsync(string key,SaveSettingRequest request,CancellationToken ct);
}
